...
def generate_image(prompt, seed=None):
    ...
    try:
        # ===== DETAILED LOGGING: Initial Prompt =====
        ...
        # ===== DETAILED LOGGING: NSFW Filtering =====
        print("\n==== NSFW FILTERING STAGE =====")
        if filtering_applied:
            print(f"NSFW FILTERING APPLIED: Yes")
            print(f"ORIGINAL PROMPT: {prompt}")
            print(f"FILTERED PROMPT: {filtered_prompt}")
            print(f"CHANGES MADE: {', '.join([f'{word} → {filtered_prompt.split()[i]}' for i, word in enumerate(prompt.split()) if i < len(filtered_prompt.split()) and word != filtered_prompt.split()[i]])}")
            prompt = filtered_prompt
        else:
            print(f"NSFW FILTERING APPLIED: No")
            print(f"PROMPT UNCHANGED: {prompt}")
        print("=================================")

        # For character preview, we should never use cache if seed is provided
        cache_key = f"{prompt}_{seed}" if seed is not None else prompt
        if cache_key in app_state.prompt_cache and os.path.exists(app_state.prompt_cache[cache_key]):
            # ===== DETAILED LOGGING: Cache Hit =====
            print("\n==== CACHE RETRIEVAL STAGE =====")
            print(f"CACHE HIT: Yes")
            print(f"CACHE KEY: {cache_key[:50]}{'...' if len(cache_key) > 50 else ''}")
            print(f"CACHED IMAGE PATH: {app_state.prompt_cache[cache_key]}")
            print("=================================")
            # Return just the filename, not the full path
            return os.path.basename(app_state.prompt_cache[cache_key]), "Retrieved from cache"

        # ===== DETAILED LOGGING: Cache Miss =====
        if cache_key in app_state.prompt_cache:
            print("\n==== CACHE RETRIEVAL STAGE =====")
            print(f"CACHE HIT: No (entry exists but file missing)")
            print(f"CACHE KEY: {cache_key[:50]}{'...' if len(cache_key) > 50 else ''}")
            print("=================================")
        else:
            print("\n==== CACHE RETRIEVAL STAGE =====")
            print(f"CACHE HIT: No (no entry found)")
            print("=================================")
        ...
        if seed is not None:
            ...
            # ===== DETAILED LOGGING: Seed Setting =====
            print("\n==== SEED CONFIGURATION =====")
            print(f"SEED VALUE: {seed}")
            print(f"GENERATOR TYPE: {'CUDA' if torch.cuda.is_available() else 'CPU'}")
            print("==============================")

        # Check if prompt is too long for CLIP (77 tokens limit)
        # Truncate if necessary to avoid the warning message
        max_length = 77 * 4  # Approximate character count (4 chars per token average)
        if len(prompt) > max_length:
            # ===== DETAILED LOGGING: Prompt Truncation =====
            print("\n==== PROMPT TRUNCATION STAGE =====")
        ...
        # ===== DETAILED LOGGING: Generation Parameters =====
        print("\n==== GENERATION PARAMETERS =====")
        print(f"FINAL PROMPT: {prompt}")
        print(f"GUIDANCE SCALE: {app_state.guidance_scale}")
        print(f"INFERENCE STEPS: {app_state.num_inference_steps}")
        print(f"CLIP SKIP: {app_state.clip_skip}")
        print(f"SCHEDULER: {app_state.scheduler_type}")
        print("==================================")
        ...
        # ===== DETAILED LOGGING: Image Saving =====
        print("\n==== IMAGE SAVING STAGE =====")
        print(f"OUTPUT FILENAME: {output_filename}")
        print(f"FULL OUTPUT PATH: {output_path}")
        print("==============================")

        # Cache the result with seed in the key if provided
        cache_key = f"{prompt}_{seed}" if seed is not None else prompt
        app_state.prompt_cache[cache_key] = output_path
        
        # Store the seed used
        if generator is not None:
            app_state.last_used_seed = seed
            
        # ===== DETAILED LOGGING: Process Complete =====
